...
<script>
  window.hirmeoswidget = ((d, t, id) => {
    ...
    s.id = id;
    s.src =
      'https://storage.googleapis.com/operas/metrics-widget-0.2.1/widget.js';
...
<link
  rel="stylesheet"
  href="https://storage.googleapis.com/operas/metrics-widget-0.2.1/widget.css"
/>
```

The `0.2.1` version number included in the JS and CSS snippets above is currently the latest version.
...
export const MetricsWidget = ({ config }) => {
  ...
  useEffect(() => {
    if (uuid) {
      ...
      widgetCSS.href = `https://storage.googleapis.com/operas/metrics-widget-0.2.1/widget.css`;
...
Lastly, you'll want to update this very README file, query for all strings containing `0.2.1` (including this one) and replace with the brand new version of the widget.
